...
{
    /// <summary>
    /// Interaction logic for DeXuatCongCong.xaml
    /// </summary>
    public partial class DeXuatCongCong : UserControl
    {
        MainChamCong Main;
        public DeXuatCongCong(MainChamCong main)
        {
            InitializeComponent();
            Main = main;
            getData();
       ...
       // List<OOP.NhanVien.DonDeXuat.CaLamViec.Item> listCaLamViec = new List<OOP.NhanVien.DonDeXuat.CaLamViec.Item>();
        ...
        private async void Border_MouseLeftButtonUp_1(object sender, MouseButtonEventArgs e)
        {
            ...
            {
                var client = new HttpClient();
                var request = new HttpRequestMessage(HttpMethod.Post, "http://210.245.108.202:3005/api/vanthu/dexuat/addDXC");
                request.Headers.Add("Authorization", "Bearer " + Properties.Settings.Default.Token);
                ...
                content.Add(new StringContent("8:00"), "time_vao_ca");
                content.Add(new StringContent("11:30"), "time_het_ca");
                content.Add(new StringContent("1"), "id_ca_xnc");
